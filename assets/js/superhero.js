...
const marvalAppLogo = document.getElementById('app-name');
const heroTab = document.getElementById('character-tab');
...
marvalAppLogo.addEventListener('click', navigateToHome);

// Function to create new DOM element for each character
const newListDom = (character, btnClass, btn) => { 											// #2
...
const toggleBetweenTab = (e) => {
  ...
  if (showFavourites && id == 'character-tab') {
    ...
    showFavourites = true;
    tab.classList.add('active-tab');
    heroTab.classList.remove('active-tab');
  }
...
const renderheros = () => { 											// #3
  ...
  if (characters.length == 0) {
    ...
    empty.id = 'empty-list';
    empty.innerHTML = 'No heros found!';
...
const fetchHeros = async () => { 												// #1
  const API_ROOT = 'https://gateway.marvel.com';
  const ts = 2;
  const publicKey = 'b632a9805d489a8364c4de9e990bd013';
  const hash = 'c97922beb53f9c80377d277d6a07196c';
  const limit = 100;
  const orderBy = 'name';
  let offset = 0;
...
const enableAutocomplete = function () {											// #at search
...
heroTab.addEventListener('click', toggleBetweenTab);
favTab.addEventListener('click', toggleBetweenTab);
